                ...
                <DivDetails>
                    <DivCloudy>
                        <TitlePrice>
                            ...
                            <TitleCloudy>{myTravel.map((travel)=>travel.airline)}</TitleCloudy>
                        </TitlePrice>
                        <H3Cloudy>{myTravel.map((travel)=>travel.locations.origin_letter)} - {myTravel.map((travel)=>travel.locations.destination_letter)}</H3Cloudy>
                        <TitlePrice>
                            ...
                            <H3CloudyOrage>{myTravel.map((travel)=>travel.cabin_class)}</H3CloudyOrage>
                        </TitlePrice>
                    </DivCloudy>
                    <DivDate>
                        
                        <H2Numero>{dayjs(myTravel.map((travel)=>travel.flight_date)).format('D')}</H2Numero>
                        <H2Fecha>{dayjs(myTravel.map((travel)=>travel.flight_date)).format('MMM')}, {dayjs(myTravel.map((travel)=>travel.flight_date)).format('YYYY')}</H2Fecha>
                    </DivDate>
                </DivDetails>
                <DivDetails>
                    <DivCloudy>
                        <DivH2>{dayjs(myTravel.map((travel)=>travel.departure_time)).format('LT')} - {dayjs(myTravel.map((travel)=>travel.arrival_time)).format('LT')}</DivH2>
                        <TitlePrice>
                            ...
                            <H3Blue>{myTravel.map((travel)=>travel.flight_duration.hours)}h{myTravel.map((travel)=>travel.flight_duration.minutes)}m</H3Blue>
                        </TitlePrice>
                    </DivCloudy>
                    <DivDirect>
                        ...
                        <TitlePrice>
                            ...
                            <H3Blue>{myTravel.map((travel)=>travel.trip_type) == 'round_trip' ? 'Direct' : 'Ecale'}</H3Blue>
